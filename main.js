...
const partnerImages = document.querySelectorAll(".partner-gallery--desktop .partner-image");
const partnerInfo = document.querySelector(".partner-info");
const partnerDesktopMQ = window.matchMedia("(min-width: 981px)");

function updatePartnerScrollGallery() {

    if (!partnerImages.length || !partnerInfo || !partnerDesktopMQ.matches) return;

    const rect = partnerInfo.getBoundingClientRect();
    const sectionTop = window.scrollY + rect.top;
    const scrollInside = window.scrollY - sectionTop;
    const maxScroll = partnerInfo.offsetHeight - window.innerHeight;
    const progress = Math.max(0, Math.min(1, scrollInside / maxScroll));
    const index = Math.min(partnerImages.length - 1, Math.floor(progress * partnerImages.length));
...
if (partnerImages.length && partnerInfo) {
    window.addEventListener("scroll", updatePartnerScrollGallery, { passive: true });
    partnerDesktopMQ.addEventListener("change", updatePartnerScrollGallery);
...
if (partnerInfo) {
    ...
    const observer = new IntersectionObserver((entries) => {
        ...
        entries.forEach(entry => {
            ...
            if (entry.isIntersecting) {
                partnerInfo.classList.add("reveal");
            }
...
const promoButton = document.querySelector("#copyPromo");

if (promoButton) {

    promoButton.addEventListener("click", () => {

        navigator.clipboard.writeText("BANTY");

        const message = document.createElement("div");

        message.className = "copy-message";
        message.textContent = "✓ Промокод BANTY скопирован";
...
const partnerSection = document.querySelector(".partner-showcase");
...
/* =====================================================
   MOBILE PARTNER SWIPER GALLERY
===================================================== */

let partnerSwiper = null;
const partnerMobileMQ = window.matchMedia("(max-width: 980px)");
...
function initPartnerSwiper() {
    ...
    if (partnerMobileMQ.matches && !partnerSwiper) {
    ...
    } else if (!partnerMobileMQ.matches && partnerSwiper) {
...
if (document.querySelector(".partner-swiper")) {
    initPartnerSwiper();
    partnerMobileMQ.addEventListener("change", initPartnerSwiper);
}